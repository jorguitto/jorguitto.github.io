...
export function mountStravaProPanel(opts) {
  ...
  const overviewPanel = `
    <div data-strava-tab-panel="overview" class="space-y-3 min-w-0">
      ${intelStrip}
      <div class="rounded-3xl border border-violet-200/90 bg-gradient-to-br from-violet-50 via-white to-white p-4 shadow-md ring-1 ring-violet-100 sm:p-5">
        <div class="flex flex-col gap-3 min-[480px]:flex-row min-[480px]:items-center min-[480px]:justify-between">
          <div class="min-w-0">
            <p class="text-[9px] font-black uppercase tracking-[0.2em] text-violet-800">Coach IA (tu API)</p>
            <p class="mt-1 text-xs leading-relaxed text-slate-700 sm:text-sm">Genera un informe semanal con tus datos importados, perfil y tendencias. Usa el proveedor y clave que guardaste en <span class="font-bold">Editar Perfil Biológico</span>.</p>
          </div>
          <button type="button" class="shrink-0 rounded-2xl bg-violet-600 px-4 py-3 text-xs font-black uppercase tracking-wide text-white shadow-lg transition hover:bg-violet-700 active:scale-[0.99]" onclick="if (window.runAiCoachStravaReport) window.runAiCoachStravaReport();">
            Informe IA
          </button>
        </div>
      </div>
