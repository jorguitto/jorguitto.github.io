/**
 * UI premium Strava: monta HTML en #strava-pro-mount (desde index.html / fittracker-app).
...
function sportDualSummaryBlock(catKey, label, agg7, aggAll, allForSpan, windowDays) {
  ...
  const graphHint =
    ' más abajo tienes la <strong>gráfica combinada</strong> (distancia, FC y ritmo o duración) con selector de <strong>agrupación</strong> (día, semana, mes, año o cada sesión); bajo la gráfica verás una <strong>conclusión automática</strong> de trayectoria.';
  const insight7 =
    agg7.count > 0
  ...
  return `
    <div data-strava-tab-panel="run" class="hidden space-y-4 min-w-0">
      ...
      <div class="rounded-2xl border border-cyan-200/80 bg-cyan-50/90 p-3 shadow-sm sm:p-4">
        ...
        <p class="mt-2 text-[10px] leading-relaxed text-cyan-900/80">En cualquier pestaña de deporte, abajo verás la <strong>gráfica combinada</strong> con agrupación configurable y una <strong>conclusión</strong> bajo la gráfica. Doble toque en el mismo punto abre la ficha (si el punto agrupa varias sesiones, abre la primera).</p>
      </div>
    ...
    const paint = (activeId) => {
      ...
      if (btxt) {
        if (activeId === 'overview') btxt.textContent = '';
          ...
          btxt.textContent = `${LANE_TITLE[activeId] || activeId}: ${arr.length} actividad(es) en catálogo. Arriba: resumen 7 días + total; abajo: gráfica combinada con agrupación (día/semana/mes/año/sesión), conclusión automática y lista filtrable.`;
        }
      }
