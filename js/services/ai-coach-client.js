...
/** OpenAI-compatible: https://g0i.ai — chat completions en /v1/chat/completions */
const G0I_OPENAI_COMPAT_BASE = 'https://g0i.ai/v1';
const OPENAI_OFFICIAL_BASE = 'https://api.openai.com/v1';

/**
 * Base URL tipo "https://…/v1" para APIs estilo OpenAI.
 * Si el navegador devuelve "Failed to fetch", suele ser CORS: despliega el Worker en
 * scripts/cloudflare-worker-openai-compat-proxy.js y asigna aquí la URL pública.
 * @param {'openai'|'g0i'} provider
 */
function resolveOpenAiCompatBase(provider) {
  const p = String(provider || '').toLowerCase().trim();
  const map =
    typeof window !== 'undefined' && window.__FITTRACKER_OPENAI_COMPAT_BASE__ && typeof window.__FITTRACKER_OPENAI_COMPAT_BASE__ === 'object'
      ? window.__FITTRACKER_OPENAI_COMPAT_BASE__
      : null;
  if (p === 'g0i') {
    const o = map && map.g0i != null ? String(map.g0i).trim() : '';
    return (o || G0I_OPENAI_COMPAT_BASE).replace(/\/$/, '');
  }
  if (p === 'openai') {
    const o = map && map.openai != null ? String(map.openai).trim() : '';
    return (o || OPENAI_OFFICIAL_BASE).replace(/\/$/, '');
  }
  return OPENAI_OFFICIAL_BASE.replace(/\/$/, '');
}

function openAiCompatChatCompletionsUrl(provider) {
  return `${resolveOpenAiCompatBase(provider)}/chat/completions`;
}
...
const DEFAULT_MODELS = {
  ...
  /** g0i.ai quick start (modelo por defecto del panel). */
  g0i: 'qwen3-coder-80b',
};
...
/** Quita espacios invisibles y comillas al copiar/pegar claves. */
...
const CLOUD_COMPLETION_TIMEOUT_MS = 120000;

async function fetchWithTimeout(url, init, timeoutMs = CLOUD_COMPLETION_TIMEOUT_MS) {
  ...
  try {
    ...
    if (e && e.name === 'AbortError') {
      throw new Error(
        `La petición superó ${Math.round(timeoutMs / 1000)} s sin respuesta (timeout). ` +
          'Comprueba la red o prueba otro proveedor o modelo.'
      );
    }
    const raw = ((e && e.message) || String(e || '')).trim();
    if (/failed to fetch|networkerror|network request failed|load failed|fetch.*abort/i.test(raw)) {
      throw new Error(
        'No se pudo conectar con el servidor de IA (suele ser CORS o red). ' +
          'Desde una página web, muchas APIs no permiten llamadas directas al dominio del proveedor. ' +
          'Solución: despliega un proxy con cabeceras CORS (ej. Cloudflare Worker; plantilla en ' +
          'scripts/cloudflare-worker-openai-compat-proxy.js) y en tu HTML define antes de los módulos: ' +
          'window.__FITTRACKER_OPENAI_COMPAT_BASE__ = { g0i: "https://TU-WORKER.workers.dev/v1" }; ' +
          'Opcionalmente openai: "…/v1" si OpenAI también falla en el navegador. Detalle: ' +
          raw
      );
    }
    throw e;
...
function formatGeminiApiError(status, rawBody) {
  ...
  if (status === 400) {
    ...
    if (invalidKey) {
      return (
        'Gemini: la API key no es válida para generativelanguage (HTTP 400, API_KEY_INVALID). ' +
        'Genera una clave en https://aistudio.google.com/apikey y pégala sin espacios ni comillas. ' +
        'En Google Cloud → Credenciales: si la clave tiene restricción de “referencias HTTP”, añade tu dominio (o localhost con el puerto exacto) o prueba “Sin restricciones” solo para depurar; las claves solo-Android/iOS fallan en el navegador. ' +
        'Tras crear o rotar la clave, pulsa Guardar en el perfil y vuelve a probar.'
      );
    }
...
export function assertProviderMatchesApiKey(provider, apiKey) {
  ...
  if (looksGoogle && p === 'g0i') {
    throw new Error('Esta clave es de Google (Gemini, empieza por AIza…). Elige **Google Gemini** como proveedor, no g0i.ai.');
  }
  if (looksAnthropic && p === 'g0i') {
    throw new Error('Esta clave es de Anthropic (sk-ant-…). Elige **Anthropic** o usa la clave de g0i.ai.');
  }
...
export async function testAiCoachConnection(cfg) {
  ...
  if (provider === 'openai') {
    const res = await fetchWithTimeout(openAiCompatChatCompletionsUrl('openai'), {
      method: 'POST',
  ...
  if (provider === 'g0i') {
    const res = await fetchWithTimeout(openAiCompatChatCompletionsUrl('g0i'), {
      method: 'POST',
      headers: { Authorization: `Bearer ${apiKey}`, 'Content-Type': 'application/json' },
      body: JSON.stringify({
        model,
    ...
    const text = await res.text();
    if (!res.ok) throw new Error(`g0i.ai ${res.status}: ${text.slice(0, 400)}`);
    return { ok: true, provider, model };
  }
  if (provider === 'gemini') {
...
export async function requestAiCoachCompletion(cfg) {
  ...
  if (provider === 'g0i') {
    ...
    const json = await res.json().catch(() => ({}));
    if (!res.ok) throw new Error(`g0i.ai ${res.status}: ${JSON.stringify(json).slice(0, 500)}`);
    const text = json.choices && json.choices[0] && json.choices[0].message && json.choices[0].message.content;
    return String(text || '').trim();
  }
  if (provider === 'gemini') {
