...
(function () {
   ...
   */
  const STRAVA_CLIENT_ID_DEFAULT = '237715';
  const STRAVA_CLIENT_SECRET_DEFAULT = '2e1fea325f4691bd714811b2e13ecaecd4bcca7';
  ...
  function buildConfigDiagnostics() {
    ...
    if (!clientSecret || clientSecret === STRAVA_CLIENT_SECRET_DEFAULT) {
      issues.push('STRAVA_CLIENT_SECRET no está configurado (sigues con placeholder).');
    }
  ...
  function getRedirectUriForEnv() {
    ...
    if (host.startsWith('127.0.0.1') || host.startsWith('localhost')) {
      // Preferimos “exact match”: si estás en /proyecto(1).html lo usamos tal cual
  ...
  async function handleStravaOAuthCallback() {
    ...
    try {
      ...
      setStatus('Error conectando Strava. Revisa Client ID/Secret y la Redirect URI.', 'err');
      cleanUrl();
    }
  }
          ...
          window.Swal.fire({
            ...
            html: `
              <div style="text-align:left;font-size:13px">
                ...
                <p style="margin-top:10px">Define tu ID real una sola vez y todos los usuarios podrán pulsar y conectar.</p>
              </div>
            `,
          });
        }
