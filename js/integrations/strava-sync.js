...
(function () {
  ...
  /** Strava permite hasta 200 por página; importamos por páginas hasta este total (máx. respaldo). */
  const STRAVA_IMPORT_MAX_ACTIVITIES = 8000;
  const STRAVA_PAGE_SIZE = 200;
  /** Clima por actividad solo en importaciones pequeñas (evita cientos de llamadas y bloqueos). */
  const STRAVA_WEATHER_ENRICH_MAX = 80;
   ...
   * @param {number} [perPage] 1–200 (Strava limita a 200 por petición).
   * @param {number} [page] página 1-based.
   */
  async function fetchStravaActivitiesArray(db, uid, perPage, page) {
  ...
  /**
   * Descarga actividades en páginas de hasta 200 hasta alcanzar maxTotal o no haya más.
   * @param {number} maxTotal
   */
  async function fetchStravaActivitiesAll(db, uid, maxTotal) {
    const cap = Math.min(Math.max(Number(maxTotal) || STRAVA_IMPORT_MAX_ACTIVITIES, 1), 12000);
    const perPage = STRAVA_PAGE_SIZE;
    let page = 1;
    const all = [];
    while (all.length < cap) {
      const chunk = await fetchStravaActivitiesArray(db, uid, perPage, page);
      if (!chunk.length) break;
      for (let i = 0; i < chunk.length && all.length < cap; i++) {
        const a = chunk[i];
        if (a && a.id) all.push(a);
      }
      if (chunk.length < perPage || all.length >= cap) break;
      page += 1;
      if (page > 400) break;
    }
    return all;
  ...
  async function fetchLatestActivity(db, uid) {
    const arr = await fetchStravaActivitiesArray(db, uid, 1, 1);
  ...
  /**
   * Clasificación UI persistida en cada actividad (`uiKind`) para alinear catálogo y pestañas
   * con `strictStravaCategory` cuando el tipo Strava es genérico o el nombre es ambiguo.
   * @returns {{ uiKind: string, typeLabel: string }}
   */
  function classifyStravaUi(name, sportType) {
    ...
    const isGenericName =
      ...
      /^entreno\b/i.test(String(name || '').trim()) ||
      /^workout\b/i.test(String(name || '').trim());

    if (
      type.includes('ride') ||
      type.includes('virtualride') ||
      type.includes('ebikeride') ||
      type.includes('emountain') ||
      type.includes('handcycle') ||
      type.includes('gravelride') ||
      type.includes('bmx') ||
      type.includes('velomobile')
    ) {
      return { uiKind: 'ride', typeLabel: 'Ciclismo' };
    }
    if (type.includes('run') || type.includes('trail') || type.includes('virtualrun')) {
    ...
    if (type.includes('walk') || type.includes('hike') || type.includes('snowshoe')) {
      return {
        uiKind: 'walk',
        typeLabel: type.includes('hike') || type.includes('senderismo') ? 'Senderismo' : 'Caminata',
      };
    }

    if (/\b(rodaje|rodillo|zwift|trainer\s*road|mtb|bici|ciclismo)\b/.test(n)) {
      return { uiKind: 'ride', typeLabel: 'Ciclismo' };
    }
    if (/\b(correr|parkrun|tempo|fartlek|interval|5k|10k|media marat|maratón|maraton)\b/.test(n)) {
      return { uiKind: 'run', typeLabel: 'Running' };
    }
    if (/\b(natación|natacion|swim\s*set)\b/.test(n)) {
      return { uiKind: 'swim', typeLabel: 'Natación' };
    }
    ...
    if (
      ...
      type.includes('stair') ||
      type.includes('stepper') ||
      type.includes('rockclimbing') ||
      type.includes('alpineski') ||
      type.includes('nordicski')
    ) {
  ...
  async function persistStravaActivities(db, uid, workouts) {
    ...
    const BATCH = 400;
    for (let start = 0; start < workouts.length; start += BATCH) {
      const slice = workouts.slice(start, start + BATCH).filter((w) => w && w.activityId);
      if (!slice.length) continue;
      try {
        const batch = db.batch();
        for (const w of slice) {
          batch.set(col.doc(String(w.activityId)), slicePayload(w), { merge: true });
        }
        await batch.commit();
      } catch (e) {
        console.warn('persistStravaActivities batch', e);
        for (const w of slice) {
          try {
            await col.doc(String(w.activityId)).set(slicePayload(w), { merge: true });
          } catch (e2) {
            console.warn('persistStravaActivities', e2);
          }
        }
      }
    }
  ...
  /**
   * Convierte actividades Strava crudas en entrenos normalizados para la app.
   * @param {object[]} activities
   */
  async function buildWorkoutsFromActivityList(activities) {
    const list = Array.isArray(activities) ? activities.filter((x) => x && x.id) : [];
    const mod = window.__FITTRACKER_MODULES__;
    const fetchWx = list.length <= STRAVA_WEATHER_ENRICH_MAX;
    const workouts = [];
    for (const a of list) {
      let wx = null;
      if (
        fetchWx &&
        mod &&
        typeof mod.getActivityWeatherContext === 'function' &&
        Array.isArray(a.start_latlng) &&
  ...
  /**
   * Descarga hasta `maxTotal` actividades (paginado Strava) y las normaliza.
   * @param {firebase.firestore.Firestore} db
   * @param {string} uid
   * @param {number} maxTotal
   */
  async function buildStravaWorkoutsFromApi(db, uid, maxTotal) {
    const activities = await fetchStravaActivitiesAll(db, uid, maxTotal);
    return await buildWorkoutsFromActivityList(activities);
  }

  /**
   * Sincroniza hasta `perPage` actividades recientes al catálogo del día (deduplicado en applyStravaWorkoutsToGym).
   * No abre el picker; pensado para auto-sync tras login / dashboard.
   */
  async function syncStravaActivityCatalog(maxActivities) {
    const n = Math.min(Math.max(Number(maxActivities) || 200, 1), STRAVA_IMPORT_MAX_ACTIVITIES);
    try {
      const { auth, db } = getFirebase();
      const user = auth.currentUser;
      if (!user) return { ok: false, reason: 'no_auth' };
      const pre = await readUserTokens(db, user.uid);
      if (!pre || !pre.access_token) return { ok: false, reason: 'no_tokens' };

      const workouts = await buildStravaWorkoutsFromApi(db, user.uid, n);
      if (!workouts.length) return { ok: true, count: 0 };

      await persistStravaActivities(db, user.uid, workouts);
      if (typeof window.applyStravaWorkoutsToGym === 'function') {
        window.applyStravaWorkoutsToGym(workouts);
      } else {
        console.warn('syncStravaActivityCatalog: applyStravaWorkoutsToGym no disponible aún');
      }
      try {
        if (typeof window.refreshStravaInsightsAndCharts === 'function') window.refreshStravaInsightsAndCharts();
      } catch (_) {}
      return { ok: true, count: workouts.length };
    } catch (e) {
      console.warn('syncStravaActivityCatalog', e);
  ...
  async function importarEntrenosStravaGym() {
    try {
      ...
      if (typeof window.setStravaGymStatus === 'function') {
        window.setStravaGymStatus(`Descargando actividades de Strava (hasta ${STRAVA_IMPORT_MAX_ACTIVITIES})…`, 'neutral');
      }

      const workouts = await buildStravaWorkoutsFromApi(db, user.uid, STRAVA_IMPORT_MAX_ACTIVITIES);
      if (!workouts.length) {
        if (typeof window.setStravaGymStatus === 'function') {
          window.setStravaGymStatus('No hay actividades en Strava.', 'neutral');
        }
        return;
      }
      ...
      showToast(`Strava: ${workouts.length} actividades importadas (máx. ${STRAVA_IMPORT_MAX_ACTIVITIES}).`);
      if (typeof window.setStravaGymStatus === 'function') {
        window.setStravaGymStatus(`Importadas ${workouts.length} actividades. Revisa el selector / catálogo.`, 'ok');
      }
    } catch (e) {
  ...
  window.syncStravaActivityCatalog = syncStravaActivityCatalog;
      ...
      try {
        if (id === 'inicio') {
          ...
          if (typeof window.syncStravaActivityCatalog === 'function') {
            await window.syncStravaActivityCatalog(500);
          }
        }
      } catch (_) {}
      return r;
  ...
  window.addEventListener('load', () => {
    ...
    // Sync inicial (al abrir la app): deltas del día + catálogo reciente
    setTimeout(() => {
      sincronizarStrava();
    }, 1800);
    setTimeout(() => {
      if (typeof window.syncStravaActivityCatalog === 'function') {
        window.syncStravaActivityCatalog(500);
      }
    }, 3200);
  });
})();
