...
(function () {
   ...
   */
  async function fetchStravaActivitiesArray(db, uid, perPage, page = 1) {
    const url = new URL('https://www.strava.com/api/v3/athlete/activities');
    url.searchParams.set('per_page', String(Math.min(200, Math.max(1, Number(perPage) || 30))));
  ...
  /**
   * Pagina la API de Strava (hasta maxActivities o 50 páginas × 200).
   */
  async function fetchStravaActivitiesAllPages(db, uid, maxActivities = 2500) {
    const cap = Math.min(10000, Math.max(50, Number(maxActivities) || 2500));
    const perPage = 200;
    const all = [];
    for (let page = 1; page <= 50 && all.length < cap; page += 1) {
  ...
  async function persistStravaActivities(db, uid, workouts) {
    ...
    const chunkSize = 400;
    for (let offset = 0; offset < workouts.length; offset += chunkSize) {
      const batch = db.batch();
      let ops = 0;
      for (let i = offset; i < Math.min(offset + chunkSize, workouts.length); i += 1) {
        const w = workouts[i];
        if (!w || !w.activityId) continue;
        const ref = col.doc(String(w.activityId));
        batch.set(
          ref,
          {
            id: w.activityId,
  ...
  async function importarEntrenosStravaGym() {
    try {
      ...
      if (typeof window.setStravaGymStatus === 'function') {
        window.setStravaGymStatus('Descargando actividades de Strava (puede tardar un poco)…', 'neutral');
      }

      const activities = await fetchStravaActivitiesAllPages(db, user.uid, 2500);
      ...
      showToast(`Strava Gym: ${workouts.length} actividades listas para seleccionar.`);
      if (typeof window.setStravaGymStatus === 'function') {
        window.setStravaGymStatus(`Importadas ${workouts.length} actividades de Strava (todas las que devolvió la API, hasta 2500).`, 'ok');
      }
    } catch (e) {
