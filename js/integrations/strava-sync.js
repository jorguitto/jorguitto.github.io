...
(function () {
  ...
  async function persistStravaActivities(db, uid, workouts) {
    ...
    for (let i = 0; i < Math.min(workouts.length, 50); i++) {
      const w = workouts[i];
  ...
  /**
   * Construye el catálogo normalizado (misma forma que el importador manual).
   * @param {firebase.firestore.Firestore} db
   * @param {string} uid
   * @param {number} perPage
   */
  async function buildStravaWorkoutsFromApi(db, uid, perPage) {
    const activities = await fetchStravaActivitiesArray(db, uid, perPage);
    const mod = window.__FITTRACKER_MODULES__;
    const workouts = [];
   ...
   */
  async function syncStravaActivityCatalog(perPage) {
    const n = Math.min(Math.max(Number(perPage) || 50, 1), 100);
    try {
      const { auth, db } = getFirebase();
      const user = auth.currentUser;
  ...
  async function importarEntrenosStravaGym() {
    try {
      ...
      const workouts = await buildStravaWorkoutsFromApi(db, user.uid, 10);
      if (!workouts.length) {
        if (typeof window.setStravaGymStatus === 'function') {
      ...
      try {
        if (id === 'inicio') {
          ...
          if (typeof window.syncStravaActivityCatalog === 'function') {
            await window.syncStravaActivityCatalog(50);
          }
        }
      } catch (_) {}
      return r;
