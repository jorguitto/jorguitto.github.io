...
/**
 * Primera y última fecha local entre sesiones (p. ej. solo running).
 * spanDays = días naturales entre la primera y la última (mínimo 1).
 */
export function stravaWorkoutsCatalogSpan(workouts) {
  ...
  if (!Number.isFinite(minT) || !Number.isFinite(maxT) || minT === Infinity) {
    return { firstDate: null, lastDate: null, spanDays: 0 };
  }
  const spanDays = Math.max(1, Math.floor((maxT - minT) / 86400000) + 1);
  return { firstDate: new Date(minT), lastDate: new Date(maxT), spanDays };
}
